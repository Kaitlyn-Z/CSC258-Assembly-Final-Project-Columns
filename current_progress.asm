...
ADDR_KBRD:
    ...
    # A palette with a lighter shade than original column for previewing column landing
    preview_palette: .word 0xff9999, 0xffdb99, 0xffff99, 0x99ff99, 0x9999ff, 0xcc99cc                             
...
### Lock the column in place & store it in grid memory
### Variables:
### - $a0 = Current X coordinate of column (topmost gem)
### - $a1 = Current Y coordinate of column (topmost gem)
...
### Restore X and Y coordinates to initial position (to generate new column)
### Variables:
...
### Perform a vertical scan on any vertically 3-in-a-row gems
### Variables:
...
# If all checks passed: Vertical 3-in-a-row -> Mark the temp_grid for deletion
...
### Delete all gems marked as matched (val=1) in temp_grid
### Variables:
### - $a0 = Current X coordinate of column (topmost gem)
### - $a1 = Current Y coordinate of column (topmost gem)
...
del_col_loop:
# Calculate index in temp_grid
mult $s2, $s0                           # Address for start of current row
mflo $t1
add $t1, $t1, $s3                       # Add the column offset
sll $t1, $t1, 2                         # Multiply by 4 to get index in memory

# Check the current temp_grid for deletion marking
la $t5, temp_grid                       # Load temp_grid memory
add $t5, $t5, $t1                       # Get the address in temp_grid
lw $t3, 0($t5)                          # Get the deletion marker (0/1)
beq $t3, $zero, skip_del                # If val is not 1, doesn't need to delete

# If grid needs deletion
# addi $sp, $sp, -4                       # Store $t1 to prevent overwriting
# sw $t1, 0($sp)

sw $zero, 0($t5)                        # Reset the temp_grid marker status
